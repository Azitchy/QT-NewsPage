      ...
      <div className="flex flex-col md:flex-row md:flex-wrap justify-center gap-6 mt-[10px] lg:mt-[40px] mx-auto max-w-7xl lg:px-4">
        <div className="flex flex-col md:flex-row md:flex-wrap justify-center gap-[15px] lg:gap-6 mt-[40px] mx-auto max-w-7xl lg:px-4">
          {newsData.map((news, index) => (
            <Card
              key={`news-${index}`}
              className="w-full md:w-[calc(33.333%-16px)] flex flex-col border border-solid border-[#eeeeee] rounded-[20px] hover:shadow-md transition-shadow h-full"
            >
              {/* Image Section */}
              <div className="flex flex-col items-start gap-2.5 p-[15px] w-full bg-[#fbfbfb] dark:bg-[#2B2F3E] rounded-t-[20px] md:min-h-[150px] lg:min-h-[200px]">
                <img
              ...
              <div className="flex flex-col items-start gap-2.5 pt-0 lg:pb-5 px-[20px] sm:px-[25px] lg:px-[30px] w-full bg-[#fbfbfb] dark:bg-[#2B2F3E] flex-grow">
                <h3 className="w-full font-normal text-[#1c1c1c] [font-family:'Inter',Helvetica] dark:text-gray-300 text-[26px] md:text-[14px] lg:text-[26px] lg:leading-8">
                  {news.title}
                </h3>
              </div>

              {/* Footer Section (always at bottom) */}
              <div className="mt-auto flex items-center justify-between px-[20px] sm:px-[20px] lg:px-[30px] py-[15px] bg-[#fbfbfb] dark:bg-[#2B2F3E] rounded-b-[20px]">
                <span className="md:text-[12px] lg:text-[16px] font-light text-[#4f5555] dark:text-gray-300">
                  {news.date}
                </span>
