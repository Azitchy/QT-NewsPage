...
const ALL_COINS: CoinOption[] = [
  { name: "LUCA", symbol: "LUCA", logo: LucaIcon },
  { name: "Bitcoin", symbol: "BTC", logo: BitcointIcon },
  { name: "Ethereum", symbol: "ETH", logo: EthereumIcon },
  { name: "Dogecoin", symbol: "DOGE", logo: DogeIcon },
  { name: "PancakeSwap", symbol: "CAKE", logo: PancakeIcon },
  { name: "XRP", symbol: "XRP", logo: XrpIcon },
  { name: "Solana", symbol: "SOL", logo: SolanaIcon },
  { name: "Chainlink", symbol: "LINK", logo: LinkIcon },
];
...
export default function AddCoinsModal({
  ...
  const generateMockPrice = () => {
    ...
    return {
      ...
      btcValue: `฿ ${(Number(price) / 50000).toFixed(5)}`,
      btcChange: `${Number(change) > 0 ? "+" : ""}${change}%`,
    };
  };
  ...
  const handleSave = async () => {
    ...
    const updated: WatchlistCoin[] = ALL_COINS.filter((c) =>
      ...
      const existing = selectedCoins.find((coin) => coin.name === c.symbol);
  ...
  const handleRemoveAllConfirm = async () => {
    ...
    const onlyLuca = selectedCoins.filter((c) => c.name === "LUCA");
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex justify-end bg-black/30">
        <div className="w-105 bg-white p-6 h-full rounded-l-2xl shadow-xl relative overflow-y-auto">
          {/* HEADER */}
