...
export const ConsensusConnection: React.FC = () => {
  ...
  const tabs = [
    { key: 'Connected', label: 'Connected', icon: CheckCircle, color: 'text-green-600 dark:text-green-400' },
    { key: 'Pending', label: 'Pending', icon: Clock, color: 'text-yellow-600 dark:text-yellow-400' },
    { key: 'Waiting', label: 'Waiting', icon: AlertCircle, color: 'text-blue-600 dark:text-blue-400' },
    { key: 'Cancelled', label: 'Cancelled', icon: XCircle, color: 'text-red-600 dark:text-red-400' },
    { key: 'Disconnected', label: 'Disconnected', icon: Unlink, color: 'text-gray-600 dark:text-gray-400' }
  ];
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      Connected: { color: 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-400 border-green-200 dark:border-green-800', icon: CheckCircle },
      Pending: { color: 'bg-yellow-100 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800', icon: Clock },
      Waiting: { color: 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-400 border-blue-200 dark:border-blue-800', icon: AlertCircle },
      Cancelled: { color: 'bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-400 border-red-200 dark:border-red-800', icon: XCircle },
      Disconnected: { color: 'bg-gray-100 dark:bg-gray-900/20 text-gray-800 dark:text-gray-400 border-gray-200 dark:border-gray-700', icon: Unlink }
    };
    ...
    return (
      <div className="flex-1 p-4 sm:p-6 bg-gray-50 dark:bg-gray-900">
        <div className="text-center py-12">
          ...
          <h2 className="text-xl font-medium text-gray-900 dark:text-gray-100 mb-2">Authentication Required</h2>
          <p className="text-gray-600 dark:text-gray-400">Please connect your wallet to manage consensus connections.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 flex-1 p-4 sm:p-6 bg-gray-50 dark:bg-gray-900">
      {/* Header */}
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-gray-100">Consensus Connection</h1>
          <button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center gap-2 px-4 py-2 bg-teal-500 dark:bg-teal-600 text-white rounded-lg hover:bg-teal-600 dark:hover:bg-teal-700 transition-colors"
          >
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
            />
          </div>
          <div className="flex gap-2">
            <button
              ...
              disabled={loading}
              className="px-4 sm:px-6 py-2 bg-teal-500 dark:bg-teal-600 text-white rounded-lg hover:bg-teal-600 dark:hover:bg-teal-700 transition-colors disabled:opacity-50"
            >
              Search
            </button>
            {isSearchMode && (
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3 sm:px-4 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
                  activeTab === tab.key
                    ? 'border-teal-500 dark:border-teal-400 text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20'
              ...
              >
                <Icon className={`w-4 h-4 ${activeTab === tab.key ? tab.color : 'text-gray-400 dark:text-gray-500'}`} />
                {tab.label}
                {tabCounts[tab.key] > 0 && (
                  <span className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400 text-xs rounded-full px-2 py-1">
                    {tabCounts[tab.key]}
                  </span>
                )}
              </button>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        {loading ? (
          <div className="p-8 text-center">
            <RefreshCw className="w-8 h-8 animate-spin text-teal-500 dark:text-teal-400 mx-auto mb-4" />
          ...
          <>
                    ...
                    <tr key={connection.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-3 sm:px-4 py-3">
                        <button
                          ...
                          className="text-teal-600 dark:text-teal-400 hover:text-teal-900 dark:hover:text-teal-300 text-sm font-medium"
                        >
                          View details
                        </button>
                      </td>
            ...
            {!isSearchMode && totalCount > itemsPerPage && (
              <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div className="flex justify-center gap-2">
                  <button
                    ...
                    className="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded text-sm disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800"
                  >
                    Previous
                  </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Create a connection</h3>
            </div>
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                >
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                <h4 className="text-sm font-medium text-blue-900 dark:text-blue-400 mb-2">Connection Instructions</h4>
            ...
            <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-teal-500 dark:bg-teal-600 text-white rounded-md hover:bg-teal-600 dark:hover:bg-teal-700 disabled:opacity-50 transition-colors"
              >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Connection details</h3>
              <button
            ...
            {detailLoading ? (
              <div className="p-8 text-center">
                <RefreshCw className="w-8 h-8 animate-spin text-teal-500 dark:text-teal-400 mx-auto mb-4" />
                <p className="text-gray-600 dark:text-gray-400">Loading connection details...</p>
              </div>
            ) : (
              <div className="p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Connection object</label>
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Lockup time</label>
                    <span className="text-sm text-gray-900 dark:text-gray-100">{connectionDetail?.lockupTime || selectedConnection.lockupTime}</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Locked position amount</label>
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Creation time</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">The counterparty's locked position amount</label>
                    <div className="text-sm text-gray-900 dark:text-gray-100">
                      {connectionDetail?.counterpartyLockedAmount || selectedConnection.counterpartyLockedAmount} LUCA
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Connection details</label>
                    <span className="text-sm text-gray-900 dark:text-gray-100">
                ...
                <div className="mb-6">
                  <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">hash</label>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded flex-1 text-gray-900 dark:text-gray-100">
                    ...
                    <a
                      ...
                      className="flex items-center gap-1 text-teal-600 dark:text-teal-400 hover:text-teal-800 dark:hover:text-teal-300 text-sm"
                    >
                ...
                <div className="mb-6">
                  <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Liquidated damage</label>
                  ...
                  {selectedConnection.connectionStatus === 'Pending' && (
                    <>
                      <button
                        ...
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-teal-500 dark:bg-teal-600 text-white rounded-md hover:bg-teal-600 dark:hover:bg-teal-700 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-2 px-4 py-2 border border-red-300 dark:border-red-600 text-red-600 dark:text-red-400 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                      >
