...
interface TabsListProps
  ...
  gradientBorder?: boolean; // dark mode gradient border
}
...
>(({ className, gradientBorder = false, ...props }, ref) => (
  <div
    className={cn(
      gradientBorder
        ? "dark:border-gradient rounded-[40px]"
        : ""
    )}
  >
    <TabsPrimitive.List
      ref={ref}
      className={cn(
        "inline-flex h-9 items-center justify-center rounded-[40px] bg-background p-1 text-muted-foreground transition-all",
        className
      )}
      {...props}
    />
