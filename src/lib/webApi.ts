...
// Don't set global withCredentials - we'll handle it per request
// axios.defaults.withCredentials = true; // REMOVED

// Helper function to determine if we should use credentials
const shouldUseCredentials = () => {
  // In development with proxy, we don't need withCredentials
  // In production, we need withCredentials for authentication
